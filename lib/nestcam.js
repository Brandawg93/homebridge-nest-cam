...
class NestCam {
  constructor(api, info, log) {
    let self = this;
    NestEndpoints.init(api.fieldTest);
    self.api = api;
    self.log = log;
  ...
  async updateInfo() {
    ...
    try {
      ...
      let response = await self.api.sendRequest(NestEndpoints.CAMERA_API_HOSTNAME, '/api/cameras.get_with_properties?' + query, 'GET');
  ...
  async toggleActive(enabled) {
    ...
    try {
      await self.api.sendRequest(NestEndpoints.CAMERA_API_HOSTNAME, '/api/dropcams.set_properties', 'POST', query);
  ...
  handleSnapshotRequest(request, callback) {
    ...
    self.api.sendPromiseRequest(self.apiHost.slice(8), '/get_image?' + query, 'GET')
      ...
      .catch((err) => {
        if (err.code === 404) {
          fs.readFile(__dirname + '/images/offline.jpg', function (err, data) {
            if (err) {
              self.log.debug(err);
              callback(err);
            } else {
  ...
  prepareStream(request, callback) {
    ...
    if (self.enabled) {
      ...
      let streamer = new NexusStreamer(self.nexusTalkHost, self.uuid, self.api.accessToken, self.log);
